...
void load_image(GtkWidget *widget, gpointer user_data)
{
    GtkWidget *fixed = user_data;
    GtkWidget *window = gtk_widget_get_parent(fixed);
    event_box = gtk_event_box_new();
    GtkWidget *image = gtk_image_new_from_file("../images/images_notes/note_croche.png");
    ...
    load = TRUE;
    //event = event_box;                                                                                                                                                              
    g_signal_connect(event_box, "button-press-event", G_CALLBACK(on_image_button_press), NULL);
    g_print("LOAD!\n");
}
    ...
    if (event->button == 1)
      {
        ...
        gtk_fixed_put(GTK_FIXED(fixed), event_box, glade_x,glade_y);
